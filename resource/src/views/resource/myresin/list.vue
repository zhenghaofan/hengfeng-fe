<template>
  <div v-if="list.length > 0"> 
    ...
    <div class="g-avai-sort f-clearfix">
      ...
      <div class="list-content">
        <p class="tit-resourcename"><a :href="'/views/resource/detail.html?status=myin&id='+item.id" v-html="item.name"></a></p>
        <div class="item-opts">
  ...
  <norecord :loading="loading" norecordTxt="当前尚无已入库的资源，加油建设吧。" v-else></norecord>
...
export default {
  data: function () {
      ...
      checkedAll: false,
      //是否有下架权限
      hasClosedAuth: true,//this.authTempList.indexOf('RESOURCE_CLOSED') !== -1 ? true : false
      loading: false,
  	};
  },
  props: ['condition'],
  ...
  methods: {
    ...
    getDetails: function (typeId, id) {
      ...
      window.location.href = preFix + 'status=myin&id=' + id;
    },
    //标星
    ...
    showConfirmBox: function (id, isChecked) {
      ...
      //全选/多选
        var tips = '您选中了'+ this.selectedIdList.length +'条资源，拟进行下架操作。';
        this.$myMsgBox({
          confirmTxt: '确认下架',
          cancelTxt: '我再想想',
          hasCancelBtn: true,
          message: '<p><i class="icon-pop-impt"></i></p><p class="g-mt20">' + tips +'</p>',
          callback: function () {
            self.getOff(self.selectedIdList);
          }
        });
    ...
    //下架
    getOff: function (selIds) {
      var self = this,
          params = selIds;

      apiUrl.getOff(params, 'my').then(function (res) {
        self.selectedIdList = [];
        self.$myMsgBox({
          message: '<p><i class="icon-pop-off"></i></p><p class="g-mt20">“资源下架”操作成功。</p>'
        });
        self.$emit('change-ckboxstatus', false);
        //更新列表
        self.gotoPage(1);
      }, function (res) {
