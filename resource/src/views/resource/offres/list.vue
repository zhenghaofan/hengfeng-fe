<template>
  ...
  <norecord :loading="loading" norecordTxt="无下架资源记录。" v-else></norecord>
...
export default {
  ...
  methods: {
    ...
    showConfirmBox: function (id, isChecked) {
      ...
      //是单个的，且当前复选框未被选中 || 全选处的上架
      if (id && !isChecked || !id && length < 1) {
        this.$message('请先选中需上架的资源~');
        return;
      }
      ...
      if (length >= 1) {
      //全选/多选
        var tips = '您选中了'+ this.selectedIdList.length +'条资源，拟进行上架操作。';
        this.$myMsgBox({
          confirmTxt: '确认上架',
          cancelTxt: '我再想想',
          hasCancelBtn: true,
          ...
          callback: function () {
            self.getOn(self.selectedIdList);
          }
        });
    ...
    //上架
    getOn: function (selIds) {
      var self = this,
          params = selIds;

      apiUrl.getOn(params).then(function (res) {
        self.selectedIdList = [];
        self.$myMsgBox({
          message: '<p><i class="icon-pop-off"></i></p><p class="g-mt20">“资源上架”操作成功。</p>'
        });
